...
- **Fully local:** Stores chats in localstorage for convenience, with optional PostgreSQL-backed memories.
- **Vision OCR ready:** Upload images and Gemma 3 1B handles image understanding and text extraction automatically.
...
Long-term and short-term memories are now backed by PostgreSQL. To run the local database:

1. Ensure Docker is installed, then start the container:
   ```
   docker compose up -d postgres
   ```
2. Copy `.env` to `.env.local` if needed and confirm `DATABASE_URL` points to `postgresql://eburon:eburon@localhost:5432/eburon_chat`.
3. When the container boots it automatically seeds the schema from `database/schema.sql`.

Chat requests will store the latest messages and maintain a rolling long-term summary, which is injected into prompts alongside retrieved document context.

# Vision OCR (Gemma 3 1B)

- `gemma3:1b` is pre-pulled in the Docker image. For local development run:
  ```
  ollama pull gemma3:1b
  ```
- When you attach an image the UI automatically switches to `gemma3:1b`, and the backend injects an OCR instruction so the model returns detected text and a brief visual description by default.
