import { CoreMessage, Message, generateId } from "ai";
import { create } from "zustand";
import { createJSONStorage, persist } from "zustand/middleware";

const VISION_MODEL = "gemma3:1b";
  ...
  persist(
    (set, get) => ({
      ...
      setBase64Images: (base64Images) =>
        set((state) => {
          const nextState: Partial<State> = { base64Images };
          if (
            base64Images &&
            base64Images.length > 0 &&
            state.selectedModel !== VISION_MODEL
          ) {
            nextState.selectedModel = VISION_MODEL;
          }
          return nextState;
        }),
